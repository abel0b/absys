...
void abl_str_stack_new(struct abl_str_stack* str_stack) {
    abl_objpool_new(&str_stack->objpool, sizeof(char), NULL);
    abl_vec_cstr_new(&str_stack->stack);
}

char* abl_str_stack_pushf(struct abl_str_stack* str_stack, char * fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
char* abl_str_stack_pop(struct abl_str_stack* str_stack) {
    char* str = abl_vec_cstr_pop(&str_stack->stack);
    abl_objpool_pop(&str_stack->objpool, strlen(str) + 1);
    return str;
}

char* abl_str_stack_peek(struct abl_str_stack* str_stack) {
    char* str = abl_vec_cstr_get(&str_stack->stack, str_stack->stack.size - 1);
    return str;
}

void abl_str_stack_del(struct abl_str_stack* str_stack) {
