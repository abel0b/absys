...
static void checkptr(void* ptr, size_t size) {
    if (ptr == NULL) {
        absys_elog("dynamic memory allocation of %zuB failed", size);
        exit(1);
    }
}
...
static size_t checklimit(size_t newmem) {
    if (newmem > memlimit) {
        absys_elog("memory limit reached");
        exit(1);
    }
    return newmem;
}
...
ABSYS_API void* absys_malloc_aux(size_t size, const char* file, int line, const char* func) {
    if (getenv("DEBUG_MEMORY")) {
        absys_ilog("mem.malloc: %zuB at %s:%d:%s", size, file, line, func);
    } 
#else
...
ABSYS_API void* absys_realloc_aux(void * ptr, size_t newsize, const char* file, int line, const char* func) {
    if (getenv("DEBUG_MEMORY")) {
        absys_ilog("mem.realloc: %p %zuB at %s:%d:%s", ptr, newsize, file, line, func);
    }
...
ABSYS_API void absys_free_aux(void * ptr, const char* file, int line, const char* func) {
    if (getenv("DEBUG_MEMORY")) {
        absys_ilog("mem.free: %p at %s:%d:%s", ptr, file, line, func);
    }
#else
